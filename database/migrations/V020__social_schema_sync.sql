-- =============================================================================
-- MIGRATION V020 - SOCIAL SCHEMA SYNC
-- Idempotent guard migration for the production social module schema expected by
-- src/services/social.server.ts. This intentionally does not modify older
-- migrations; it repairs databases that stopped at the original V007 social
-- schema or partially applied the V019 social management migration.
-- =============================================================================
...
SET status = CASE
  WHEN disconnected_at IS NULL THEN 'connected'
  ELSE 'disconnected'
END
...
UPDATE social_posts
SET caption = COALESCE(NULLIF(caption, ''), content, title, 'Untitled social post'),
    content = COALESCE(NULLIF(content, ''), caption, title, 'Untitled social post'),
    platform = COALESCE(platform, 'multi')
WHERE caption IS NULL OR caption = '' OR content IS NULL OR content = '' OR platform IS NULL;
...
ALTER TABLE social_posts DROP CONSTRAINT IF EXISTS chk_social_posts_caption_not_empty;
...
INSERT INTO social_post_accounts (post_id, social_account_id, platform, status, platform_post_id, published_at)
SELECT sp.id, sp.social_account_id, sp.platform,
       CASE WHEN sp.status IN ('draft', 'scheduled', 'publishing', 'published', 'failed') THEN sp.status ELSE 'draft' END,
       sp.platform_post_id, sp.published_at
FROM social_posts sp
WHERE sp.social_account_id IS NOT NULL
