  ...
  try {
    ...
    return {
      ...
      data: rows.map((r) => ({
        ...
        name: r.name,
        email: r.email,
        role: r.role_name as WorkspaceRole,
  ...
  try {
    ...
    return {
      ...
      data: rows.map((r) => ({
        ...
        invitedAt: new Date(r.created_at).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
        }),
...
export async function inviteMember(
  ...
  try {
    ...
    if (roleRows.length === 0) {
      return {
        success: false,
        error: `Role '${role}' not found in this workspace`,
        status: 400,
      };
    }
    ...
    if (existing.length > 0) {
      return {
        success: false,
        error: `A pending invitation already exists for ${normalEmail}`,
        status: 409,
    ...
    if (memberRows.length > 0) {
      return {
        success: false,
        error: `${normalEmail} is already a member of this workspace`,
        status: 409,
        code: "ALREADY_MEMBER",
      };
    }
   ...
   WHERE u.id = $1
    ...
    await createNotification({
      ...
      description: `${normalEmail} was invited to join ${workspaceName}`,
      actorId: invitedById,
...
export async function changeMemberRole(
  ...
  try {
    ...
    if (role === "Owner") {
      return {
        success: false,
        error: "Cannot assign Owner role via this method",
        status: 403,
      };
    }
    ...
    const { rows: memberRows } = await db.query<{ user_id: string }>(
    ...
    if (memberRows[0]?.user_id) {
      await createNotification({
        ...
        description: `Your workspace role was updated to ${role}`,
        actorId: requesterId,
        entityType: "team",
        entityId: memberId,
        priority: "normal",
